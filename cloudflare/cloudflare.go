...
func New(ttl int, conf Cloudflare, Http *http.Client) (*DnsProvider, error) {
	...
	if p.Zone == "" {
		return nil, errors.New("cloudflare zone can't be empty")
	}
	if p.Token == "" {
		return nil, errors.New("cloudflare token can't be empty")
	}
...
func (a DnsProvider) SetDomainRecord(domain, Type, ip string) error {
